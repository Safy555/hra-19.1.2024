...
var
  ch:char;
  i,j,k,l,poc,stavhry,hracX,hracY:integer;
...
procedure generujhru(nn,vyska,sirka:integer);   //generovanie pola
begin
   //vynulovanie pola
   for i:=0 to vyska+1 do begin
    for j:=0 to sirka+1 do begin
      p[i,j].value:=0;p[i,j].state:=FALSE;
    end;
  end;
   hracX:=2;
   hracY:=2;
...
procedure vypis(vyska,sirka,pocetmin,hracXX,hracYY:integer);    //vypis hracieho pola
begin
  clrscr;
  gotoxy(4,1);writeln('_____');
  gotoxy(4,2);writeln('|000|');
  if pocetmin<10 then gotoxy(7,2)
  else if pocetmin<100 then gotoxy(6,2)
  else gotoxy(5,2);
  write(pocetmin);
  gotoxy(4,3);
  ...
  gotoxy((hracXX*3)+3,hracYY+3);write('P');
end;
...
begin
  repeat
     vypis(vyska,sirka,pocetmin,hracX,hracY);
     gotoxy(sirka div 2,vyska+6);
     write('Pohybuj sa pomocou WASD:  ');
     repeat                              //skontrolovanie vstupu
           ch:=readkey; ch:=upcase(ch);
     until (ch='W') or (ch='A') or (ch='S') or (ch='D') or (ch='F') or (ord(ch)=13);
     case ch of
     'W':if hracY<>1 then hracY:=hracY-1;
     'A':if hracX<>1 then hracX:=hracX-1;
